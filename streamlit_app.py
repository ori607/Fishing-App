...
with st.sidebar:
    ...
    st.divider()
    # FIX #1: Global Location Selector (Accessible by all tabs)
    st.header("📍 Current Trip")
    all_site_names = list(SITES.keys())
    st.session_state['selected_spot'] = st.selectbox("Select location to analyze:", all_site_names)
...
with tabs[1]:
    ...
    if spot_name:
        ...
        # FIX #4: Current Date/Time Stamp
        now = datetime.now().strftime("%Y-%m-%d %I:%M %p")
        st.caption(f"Gathering environmental data for {spot_name} as of: **{now}**")
        
        # FIX #2: All factors (Bite score high font, others small)
        ...
        # FIX #3 & #5: Lure Recommendations with Details and Images
        st.divider()
        ...
        with col_left:
            ...
            if user_lures:
                ...
                best_owned = user_lures[0]
                if cover == "LilyPads" and "Frog" in user_lures: best_owned = "Frog"
                
                st.write(f"**Primary Choice:** 5-inch Dark Green Pumpkin {best_owned}")
                if "Senko" in best_owned:
                    st.image("https://images.tacklewarehouse.com/fishing/Gary_Yamamoto_Senko.jpg", width=200)
                    
                elif "Frog" in best_owned:
                    
            else:
                st.write("Locker is empty!")

        with col_right:
            st.subheader("🎣 Pro Suggestion (Buy This)")
            # Suggest a lure even if they don't have it
            if cover == "Rocks":
                st.write("**Recommended:** 3/8oz Black/Blue Football Jig")
                
            elif cover == "LilyPads":
                st.write("**Recommended:** White 1/2oz Hollow Body Frog")
                
            else:
                st.write("**Recommended:** 1/4oz White/Silver Whopper Plopper")
